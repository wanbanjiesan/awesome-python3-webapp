...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def find(cls, primarykey):
        ...
        print('sql: %s, primarykey: %s' % (cls.__select__, cls.__primary_key__))
...
if __name__ == '__main__':
    ...
    async def test():
        await create_pool(loop, host='127.0.0.1', port=3306,
                          user='fq', password='fq',
                          db='fq')
        u = User(id=10, name='peic')
        print(u)
        await u.save()

        userFind = await u.find(2)
        print(userFind)
